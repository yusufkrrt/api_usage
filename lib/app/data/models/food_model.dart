...
class Result {
    ...
    factory Result.fromJson(Map<String, dynamic> json) => Result(
        ...
        gaps: gapsValues.map[json["gaps"]] ?? Gaps.NO,
        ...
        creditsText: creditsTextValues.map[json["creditsText"]] ?? CreditsText.FOODISTA_COM,
        license: json["license"],
        sourceName: sourceNameValues.map[json["sourceName"]] ?? SourceName.FOODISTA,
        pricePerServing: json["pricePerServing"]?.toDouble() ?? 0.0,
        summary: json["summary"] ?? '',
        cuisines: List<String>.from((json["cuisines"] ?? []).map((x) => x)),
        dishTypes: List<DishType>.from((json["dishTypes"] ?? []).map((x) => dishTypeValues.map[x]).whereType<DishType>()),
