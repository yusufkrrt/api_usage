...
class MusicDetailView extends GetView<MusicDetailController> {
      ...
      body: controller.obx(
        (song) => SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              _buildArtwork(song!.artworkUrl100),
              const SizedBox(height: 32),
              _buildSongInfo(song.trackName, song.artistName, song.collectionName),
              const SizedBox(height: 24),
              _buildDetailCard(song),
            ],
          ),
        ),
        onLoading: const Center(child: CircularProgressIndicator()),
        onEmpty: const Center(
          child: Text('Detay bulunamadı', style: TextStyle(color: Colors.white)),
        ),
        onError: (error) => Center(
          child: Text(error ?? 'Bir hata oluştu',
  ...
  Widget _buildArtwork(String? artworkUrl100) {
    final artworkUrl = artworkUrl100?.replaceAll('100x100', '600x600');
  ...
  Widget _buildSongInfo(String? trackName, String artistName, String? collectionName) {
    return Column(
      children: [
        Text(
      ...
      child: Column(
        children: [
          ...
          _buildDetailRow(
            ...
            'Release Date',
            _formatDate(song.releaseDate?.toString()),
          ),
          _buildDivider(),
          _buildDetailRow(
            Icons.explicit,
            'Content',
            song.trackExplicitness == Explicitness.EXPLICIT ? 'Explicit' : 'Clean',
          ),
        ],
      ),
    );
